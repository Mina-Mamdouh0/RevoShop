...
class ModalSheetCart extends StatefulWidget {
  ...
  ModalSheetCart({Key key, this.product, this.type, this.loadCount}) : super(key: key);
...
class _ModalSheetCartState extends State<ModalSheetCart> {
  ...
  void addCart(ProductModel product) async {
    ...
    if (!Session.data.containsKey('cart')) {
      ...
      List<ProductModel> listCart =
          products.map((product) => new ProductModel.fromJson(product)).toList();
      ...
      if (index != -1) {
        productCart.cartQuantity = listCart[index].cartQuantity + productCart.cartQuantity;
        ...
        setState(() {
          ...
          if (value['data']['wholesales'] != null &&
            ...
            if (value['data']['wholesales'][0]['price'].isNotEmpty &&
              ...
              variationPrice = double.parse(value['data']['wholesales'][0]['price']);
            }
          }
  ...
  initVariation() {
    if (widget.product.attributes.isNotEmpty && widget.product.type == 'variable') {
        ...
        if (element.variation == true) {
          ...
          setState(() {
            ...
            if (element.id != 0 && element.id != null) {
              var _value = element.options.first.toString().replaceAll('.', '-');
              ...
              variation.add(new ProductVariation(
                  id: element.id, value: element.options.first, columnName: element.name));
            }
          });
        }
      });
                  ...
                  children: [
                    GestureDetector(
                          ...
                          setState(() {
                            if (attributes[index].id != 0) {
                              attributes[index].selectedVariant =
                                  attributes[index].options[i].toString().toLowerCase();
                            } else {
                              attributes[index].selectedVariant =
                          ...
                          variation.forEach((element) {
                            if (element.id != 0) {
                              if (element.columnName ==
                                ...
                                setState(() {
                                  var _value =
                                      attributes[index].options[i].replaceAll('.', '-');
                                  element.value = slugify(_value).replaceAll('--', '-');
                                });
                              }
                            } else {
                        ...
                        child: attributes[index].id == 0
                            ...
                            : sizeButtonPA(attributes[index].options[i], index, i)),
                    Container(
                      width: 10,
                    ),
                  ],
  ...
  Widget colorButton(String color, int indexes) {
    return Container(
      decoration: BoxDecoration(
        border:
            Border.all(color: indexColor == indexes ? Colors.transparent : secondaryColor),
        borderRadius: BorderRadius.circular(5),
      ...
      child: Text(
        color,
        style: TextStyle(color: indexColor == indexes ? Colors.white : primaryColor),
      ),
    );
  }
      ...
      decoration: BoxDecoration(
        border: Border.all(
            color: attributes[groupVariant].selectedVariant.toLowerCase() ==
                    attributes[groupVariant].options[subVariant].toString().toLowerCase()
                ? Colors.transparent
                : secondaryColor),
        borderRadius: BorderRadius.circular(5),
        color: attributes[groupVariant].selectedVariant.toLowerCase() ==
