...
class SearchProvider with ChangeNotifier {
  ...
  Future<bool> searchProducts(String search, int page) async {
    ...
    await ProductAPI().searchProduct(search: search, page: page).then((data) {
      if (data.statusCode == 200) {
        ...
        loadVariationData(load: loadingSearch, listProduct: listTempProducts).then((value) {
  ...
  Future<bool> scanProduct(String code, context) async {
    ...
    await ProductAPI().scanProductAPI(code).then((data) {
      if (data['id'] != null) {
        ...
        loadingQr = false;
        Navigator.pop(context);
        snackBar(context, message: "Product not found", color: changeColor);
      }
      loadingQr = false;
      notifyListeners();
    });
  ...
  Future<bool> loadVariationData({List<ProductModel> listProduct, bool load}) async {
