  ...
  Future<Map<String, dynamic>> setWishlistProduct(context, {productId}) async {
    ...
    await WishlistAPI().setWishlist(productId).then((data) {
      ...
      if (result['message'] == 'success') {
        if (result['type'] == 'add') {
          snackBar(context,
              message: AppLocalizations.of(context).translate('wishlist_add_message'));
        } else {
          snackBar(context,
              message: AppLocalizations.of(context).translate('wishlist_remove_message'));
        }
      } else {
        snackBar(context,
            message: AppLocalizations.of(context).translate('error_submit_message'),
            color: changeColor);
      }
